...
{
    [Test]
    public void Should_be_able_to_skip_when_decompression_is_not_required()
    {
        Should_be_able_to_skip_when_decompression_is_not_required_async(true).GetAwaiter().GetResult();
    }

    [Test]
    public async Task Should_be_able_to_skip_when_decompression_is_not_required_async()
    {
        await Should_be_able_to_skip_when_decompression_is_not_required_async(false);
    }

    private async Task Should_be_able_to_skip_when_decompression_is_not_required_async(bool sync)
    {
    ...
    [Test]
    public void Should_be_able_to_decompress_message()
    {
        Should_be_able_to_decompress_message_async(true).GetAwaiter().GetResult();
    }

    [Test]
    public async Task Should_be_able_to_decompress_message_async()
    {
        await Should_be_able_to_decompress_message_async(false);
    }

    private async Task Should_be_able_to_decompress_message_async(bool sync)
    {
        ...
        if (sync)
        {
            ...
            compressionAlgorithm.Verify(m => m.Decompress(It.IsAny<byte[]>()), Times.Once);
        }
        else
        {
            await pipeline.ExecuteAsync();
